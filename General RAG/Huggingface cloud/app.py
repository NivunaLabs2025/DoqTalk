...
try:
    ...
    st.warning(f"⚠️ Could not load LLaMA (reason: {e}). Falling back to smaller model.")
    qa_pipeline = pipeline(
        "text-generation",
        model="distilgpt2",
        device=-1
...
st.title("🧬 Biology Textbook Chatbot")
st.write(f"Using **{model_name}** as backend")
st.write("Ask me questions from **biologytext.pdf** 📘")
